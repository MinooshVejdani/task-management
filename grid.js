...
const defaultColumnClassesName = ['Task', 'Status', 'Deadline', 'Time', 'Priority', 'Checkbox'];
let columnsTypeList = [];
let rowList = [];
...
export class Grid {
    ...
    addColumn(type) {
        const id = `id-${Date.now().toString(36) + Math.random().toString(36).substring(2, 5)}`;
        ...
        for(let i = 0; i < this.rowsCount; i++){
            const gridCell = new GridCell(type, 'white', 'white', 1);
    ...
    addRow() {
        ...
        columnsElement.forEach((columnElement, index) => {
            const gridCell = new GridCell(columnsTypeList[index].type, 'white', 'white', 1);
            columnElement.appendChild(gridCell.element);
        });
        this.rowsCount++;
    }
